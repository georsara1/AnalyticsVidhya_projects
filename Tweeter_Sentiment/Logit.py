...
for i in range(5):
    ...
    valid_sentiments = clf.predict_proba(valid_x)
    valid_sentiments = valid_sentiments[:,0]
    valid_sentiments[valid_sentiments<0.9]=0
    valid_sentiments[valid_sentiments>0.9]=1
...
model.add(LSTM(96, dropout=0.4, recurrent_dropout= 0.2))
...
df_submission.to_csv('lstm1.csv', index = False)
